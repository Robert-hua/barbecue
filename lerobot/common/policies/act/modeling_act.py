...
class DINOv2BackBone(nn.Module):
    ...
    def forward(self, tensor):
        ...
        od = OrderedDict()
        od["feature_map"] = xs.reshape(xs.shape[0], 22, 16, 384).permute(0, 3, 2, 1)
        return od
...
class ACTPolicy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        """
        self.eval()
        if self.config.vision_backbone.startswith("dino"):
            for key in self.config.image_features:
                batch[key] = v2.Resize((16 * 14, 22 * 14))(batch[key])
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.image_features:
            ...
            elif self.config.vision_backbone.startswith("dino"):
        ...
        if self.config.image_features:
            if self.config.vision_backbone.startswith("resnet"):
                self.encoder_img_feat_input_proj = nn.Conv2d(
                    self.backbone.backbone.fc.in_features, config.dim_model, kernel_size=1
                )
            elif self.config.vision_backbone.startswith("dino"):
                self.encoder_img_feat_input_proj = nn.Conv2d(
